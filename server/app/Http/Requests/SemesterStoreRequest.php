...
class SemesterStoreRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            'semester_name' => [
                ...
                'string',
                'max:255',
                Rule::unique('Semester', 'SemesterName')->where(
                    fn ($query) => $query->where('AcademicYear', $this->input('academic_year'))
                ),
            ],
            ...
            'start_date' => [
                ...
                function (string $attribute, mixed $value, \Closure $fail) {
                    if (!$this->isOverlapped((string) $value, (string) $this->input('end_date'))) {
                        return;
                    }
    ...
    private function isOverlapped(string $startDate, string $endDate): bool
    {
